...
def step(elves_map, priorities):
    ...
    return next_elves_map, next_priorities
...
for i in range(10):
    elves_map, priorities = step(elves_map, priorities)

elves_x = {i[0] for i in elves_map}
elves_y = {i[1] for i in elves_map}

print((max(elves_x) - min(elves_x) + 1) * (max(elves_y) - min(elves_y) + 1) - len(elves_map))
