...
def get_best_flow(valve_path, remaining_time, valves):
    ...
    best_flow_amount = 0
    if remaining_time <= 1:
        best_flows[(valve_path, remaining_time)] = 0
        return 0
    ...
    for valve_name in openable_valves:
        ...
        flow_amount = self_flow_amount + get_best_flow((*valve_path, valve_name), next_remaining_time - distance, valves)
        best_flow_amount = max(flow_amount, best_flow_amount)

    best_flows[(valve_path, remaining_time)] = best_flow_amount
    return best_flow_amount
...
print(get_best_flow(('AA',), 30, valves))
